...
function testLayer1Foundation(): TestSuite {
  ...
  tests.push({
    ...
    passed: deps["tailwindcss"]?.startsWith("4.") || deps["tailwindcss"]?.includes("4."),
  })
...
function testLayer3Messaging(): TestSuite {
  ...
  tests.push({
    ...
    passed: pushActionsTs.includes('web-push') || pushActionsTs.includes('webpush'),
  })
...
function printResults(suites: TestSuite[]) {
  ...
  console.log("=".repeat(60))
  const overallColor = totalPassed === totalTests ? "green" : totalPassed > totalTests / 2 ? "yellow" : "red"
