...
const composeSchema = z.object({
  ...
  if (data.audienceType === "groups" && data.selectedGroupIds.length === 0) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Select at least one group", path: ["selectedGroupIds"] })
  }
  if (data.audienceType === "event" && !data.selectedEventId) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Select an event", path: ["selectedEventId"] })
  }
  if (data.deliveryMode === "schedule") {
    if (!data.scheduledDate) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Select a date", path: ["scheduledDate"] })
    }
    if (!data.scheduledTime) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Select a time", path: ["scheduledTime"] })
    }
    if (data.scheduledDate && data.scheduledTime) {
      ...
      if (scheduled.getTime() < minTime) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Scheduled time must be at least 5 minutes in the future", path: ["scheduledTime"] })
      }
    }
  }
})
...
export function useComposeForm() {
  ...
  const toScheduledTimestamp = (values: ComposeFormValues): number => {
    if (!values.scheduledDate || !values.scheduledTime) throw new Error("Missing schedule info")
