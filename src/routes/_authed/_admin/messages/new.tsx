...
function NewMessagePage() {
  ...
  const { form, validateStep, toCreatePayload, toScheduledTimestamp } = useComposeForm()

  const createMutation = useMutation({
    mutationFn: async (payload: Parameters<typeof convex.mutation<typeof api.messages.create>>[1]) => {
  ...
  const scheduleMutation = useMutation({
    mutationFn: async ({ id, scheduledFor }: { id: string; scheduledFor: number }) => {
      await convex.mutation(api.messages.schedule, { id: id as any, scheduledFor })
    },
  })

  const isSubmitting = createMutation.isPending || sendNowMutation.isPending || scheduleMutation.isPending
  ...
  const handleSchedule = async () => {
    ...
    const messageId = await createMutation.mutateAsync(payload as any)
    await scheduleMutation.mutateAsync({ id: messageId as string, scheduledFor })
    router.navigate({ to: "/messages" })
  }
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[#1E1B4B]/60 mt-1">Compose and send a message to your organisation.</p>
        </div>
      </div>
