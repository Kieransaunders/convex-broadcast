...
- Setting up auth providers (Convex Auth, Clerk, WorkOS AuthKit, Auth0, Better Auth, custom JWT)
...
- [Better Auth](https://labs.convex.dev/better-auth) - use when the app wants full-featured auth (email/password, OAuth, 2FA, magic link, passkeys) stored in Convex with no external auth SaaS
...
- Dependencies such as `@convex-dev/better-auth`, `better-auth`, `@clerk/*`, `@workos-inc/*`, `@auth0/*`, or Convex Auth packages
- Existing files such as `convex/betterAuth/`, `convex/auth.config.ts`, auth middleware, provider wrappers, or login components
- Environment variables that clearly point at a provider (`BETTER_AUTH_SECRET`, `CLERK_*`, etc.)
...
- Better Auth: [official docs](https://labs.convex.dev/better-auth), then `references/better-auth.md`
