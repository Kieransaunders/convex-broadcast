...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const fromEmail = await getSetting(ctx, "sender_email", "noreply@orgcomms.app")
    const appName = await getSetting(ctx, "app_name", "Org Comms")
    const siteUrl = process.env.SITE_URL || "http://localhost:3000"

    console.log(`Sending invite email to ${args.email} via Resend (From: ${fromEmail})...`)
    try {
...
export const listPending = query({
  ...
  handler: async (ctx) => {
    ...
    return pendingInvites.filter(invite => !userEmails.has(invite.email.toLowerCase()))
  },
})
